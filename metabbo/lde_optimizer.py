...
class LDE_Optimizer(Learnable_Optimizer):
    ...
    def __get_cost(self, batch, pop):
        ...
        cost = []
        for p in range(bs):
            cost.append(evaluate_problem(batch[p], pop[p]))
    ...
    def __get_feature(self, problem):
        if self.fe is not None:
            input_net = extract_features(self.fe, self.__pop.squeeze(0), self.__fit.squeeze(0)).mean(1)
        else:
    ...
    def update(self, action, problem):
        ...
        if is_done:
            finalize_cost_history(self.cost, self.gbest_cost, self.__config.n_logpoint)
