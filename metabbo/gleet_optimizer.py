...
from metabbo.optimizer_common import (
    apply_ela_count,
    count_evaluations,
    evaluate_problem,
    finalize_cost_history,
    maybe_record_cost,
    reset_cost_history,
    safe_cost_scale,
)
import torch
import numpy as np
from metabbo.feature_utils import extract_features, get_ela_feature
...
class GLEET_Optimizer(Learnable_Optimizer):
    ...
    def initialize_particles(self, problem):
        ...
        # record
        self.max_cost=safe_cost_scale(c_cost)
    ...
    def init_population(self, problem):
        ...
        reset_cost_history(self, self.particles['gbest_val'])
        # get state
        if self.fe is None:
            ...
            state = extract_features(self.fe, self.particles['current_position'], self.particles['c_cost'])[0]   # ps, node_dim
    ...
    def _get_ela_feature(self, problem):
        ...
        consumed_fes = ela[1]
        self.fes = apply_ela_count(self.__config, self.fes, consumed_fes)
    ...
    def get_costs(self,position, problem):
        self.fes += count_evaluations(position)
        return evaluate_problem(problem, position)
    ...
    def update(self, action, problem):
        ...
        if self.boarder_method=="clipping":
            ...
            raw_position=self.particles['current_position']+new_velocity
            filter=(raw_position < problem.lb) | (raw_position > problem.ub)
        ...
        if self.fe is None:
            ...
            next_state = extract_features(self.fe, self.particles['current_position'], self.particles['c_cost'])[0]
        ...
        maybe_record_cost(self, self.fes, self.particles['gbest_val'])

        if is_end:
            finalize_cost_history(self.cost, self.particles['gbest_val'], self.__config.n_logpoint)
