...
from metabbo.feature_utils import extract_features, features_to_numpy, get_ela_feature
...
class DE_DDQN_Optimizer(Learnable_Optimizer):
    ...
    def init_population(self, problem):
        ...
        self.__cost = evaluate_problem(problem, self.__X)
        # reset records
        self.fes = self.__NP
        self.__gen = 0
        self.__pointer = 0
        ...
        reset_cost_history(self, self.__c_gbest)
    ...
    def __get_state(self, problem):
        if self.fe is None:
            ...
            if self.__pointer == 0:
                self.features = features_to_numpy(extract_features(self.fe, self.__X, self.__cost)[0])
    ...
    def update(self, action, problem):
        ...
        trial_cost = evaluate_problem(problem, trial)
        self.fes += 1
        # compute reward
        ...
        maybe_record_cost(self, self.fes, self.__c_gbest)
        ...
        if is_done:
            finalize_cost_history(self.cost, self.__c_gbest, self.__config.n_logpoint)
